...
class MorphEngine {
    ...
    createFaceMask(landmarks, width, height) {
        ...
        // Erode the hull slightly inward (90% scale) to keep blend away from edges
        const erosionFactor = 0.92;
        ...
        // Pass 3: Small blur for smooth gradient
    ...
    colorCorrect(srcData, targetData, mask, width, height) {
        ...
        // Color correction factors (blend target towards source colors)
    ...
    morphFace(srcImageData, targetImageData, srcLandmarks, targetLandmarks, alpha, outputData) {
        try {
            ...
            for (let i = 0; i < outputData.data.length; i += 4) {
                ...
                // Use quadratic falloff for smoother edge transitions
                const smoothMask = maskValue * maskValue;
                const blendFactor = smoothMask * alpha;
